...
for (let i = 0; i < nodeCount; i++) {
  ...
  // Pfade für public/secret key
  const pkPath = path.join(dir, `node${i}_pk.txt`);
  const skPath = path.join(dir, `node${i}_sk.txt`);
  const node = new MW_SNP_Node(pkPath, skPath, []);
  nodes.push(node);

  // Public‑Key lesen
  allPubKeys[i] = fs.readFileSync(pkPath, "utf8").trim();
}
...
nodes.forEach((node, idx) => {
  node.connections = getRandomPeers(idx);
});
...
console.log(
  `Gestartet ${nodeCount} Nodes auf Ports ${basePort}–${
    basePort + nodeCount - 1
  }`
);
...
setInterval(() => {
  ...
  const [host, portStr, recipientPk] =
    node.connections[Math.floor(Math.random() * node.connections.length)].split(
      ":"
    );
